        ...
        try await healthKit.streamWorkouts(from: since, until: until) { workouts in
            for batch in workouts.chunked(into: batchSize) {
                ...
                for w in batch {
                    ...
                    // queryAggregatedStatistics falls back to HKSampleQuery bucketing
                    // when HKStatisticsCollectionQuery hits Apple's
                    // "Unable to invalidate interval: no data source available" bug.
                    // Isolate failures per workout so one bad window cannot fail the category.
                    ...
                    if w.duration > 0 {
                        do {
                            ...
                            print("Workout HR aggregation failed for \(w.uuid): \(error.localizedDescription); uploading workout without HR")
                            hrData = nil
                        }
                    }

...
final class HealthKitService {
    ...
    /// Prefer HKStatisticsCollectionQuery; on Apple's known
    /// "Unable to invalidate interval: no data source available" (and similar)
    /// failures, fall back to HKSampleQuery + in-memory bucketing so callers
    /// (especially workout HR) still get min/avg/max data.
    func queryAggregatedStatistics(
        ...
        do {
            return try await queryAggregatedStatisticsViaCollection(
                typeID: typeID, unit: unit, from: startDate, until: endDate, interval: interval
            )
        } catch {
            return try await queryAggregatedStatisticsViaSamples(
                typeID: typeID, unit: unit, from: startDate, until: endDate, interval: interval
            )
        }
    }

    private func queryAggregatedStatisticsViaCollection(
        typeID: HKQuantityTypeIdentifier,
        unit: HKUnit,
        from startDate: Date,
        until endDate: Date,
    ...
    private func queryAggregatedStatisticsViaSamples(
        typeID: HKQuantityTypeIdentifier,
        unit: HKUnit,
        from startDate: Date,
        until endDate: Date,
        ...
        // Match collection-query bucketing: midnight-anchored fixed intervals.
        let anchor = Calendar.current.startOfDay(for: startDate)
        var accum: [Date: (min: Double, max: Double, sum: Double, count: Int)] = [:]

        for sample in samples {
            let value = sample.quantity.doubleValue(for: unit)
            let offset = sample.startDate.timeIntervalSince(anchor)
            let bucketIndex = floor(offset / interval)
            let bucketStart = anchor.addingTimeInterval(bucketIndex * interval)
            if bucketStart >= endDate || bucketStart.addingTimeInterval(interval) <= startDate {
                continue
            }
            if var existing = accum[bucketStart] {
                existing.min = Swift.min(existing.min, value)
                existing.max = Swift.max(existing.max, value)
                existing.sum += value
                existing.count += 1
                accum[bucketStart] = existing
            } else {
                accum[bucketStart] = (min: value, max: value, sum: value, count: 1)
            }
        }

        return accum.keys.sorted().compactMap { key in
            guard let a = accum[key], a.count > 0 else { return nil }
            return AggregatedBucket(
                startDate: key,
                min: a.min,
                avg: a.sum / Double(a.count),
                max: a.max
            )
        }
    }

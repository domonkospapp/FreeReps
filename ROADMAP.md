...
|---|---|---|---|---|
| `[open]` | Continuous aggregate for the daily metric series | `server/migrations/`, `server/internal/storage/dashboard.go` | The ~0.66s floor becomes a complaint again | The front page went from ~5s to ~0.68s at 30d and ~0.97s at 1y across four changes (`a3c634c`, `5a75915`, `8c1ed3d`, plus the homelab memory fix). **Reconsider before building this:** the last round established that planning, not execution, carried the cost — the same queries run in 105ms and 9ms locally. A pre-computed table queried with bind parameters would have inherited the same planning problem, so this was never the fix it looked like. What remains unexplained is a ~0.66s floor that is flat across windows and does not match local timings; suspect the 2 CPU / spinning-storage LXC before writing any SQL. **The dedup cannot go into the view** — source priority is user-configurable, so the aggregate would group by `(day, metric_name, source)` and resolve at read time, and it needs `sum`, `avg` and `count` per bucket. |

...
|---|---|---|---|---|
| `[open]` | Continuous aggregate for the daily metric series | `server/migrations/`, `server/internal/storage/dashboard.go` | Needs a local TimescaleDB to test against | The front page went from ~5s to ~0.7s by bounding both queries in time (`a3c634c`, `5a75915`), and the remaining cost now scales with the window: 0.84s at 1d, 1.72s at 1y, measured as medians over 8 runs. Pre-computing the daily buckets makes that a read of finished rows instead. **The dedup cannot go into the view** — source priority is user-configurable, so the aggregate has to group by `(day, metric_name, source)` and resolve priority at read time. It also needs `sum`, `avg` and `count` per bucket, because cumulative metrics sum per day and the rest average. **Do not deploy this untested:** the initial backfill runs over 4.7M rows and can lock; every other change so far was revertible in seconds, this one is not. |
